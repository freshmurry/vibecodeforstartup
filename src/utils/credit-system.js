...
export function analyzePromptComplexity(prompt, context) {
    ...
    const hasCodeGenKeywords = codeGenKeywords.some(keyword => prompt.toLowerCase().includes(keyword.toLowerCase()));
    const hasRefactorKeywords = refactorKeywords.some(keyword => prompt.toLowerCase().includes(keyword.toLowerCase()));
    const hasComplexKeywords = complexKeywords.some(keyword => prompt.toLowerCase().includes(keyword.toLowerCase()));
    const hasMediumKeywords = mediumKeywords.some(keyword => prompt.toLowerCase().includes(keyword.toLowerCase()));
 ...
 */
export function getCreditCost(complexity, planType = 'free') {
 ...
 */
export function hasEnoughCredits(currentCredits, requiredCredits) {
...
export function estimateMonthlyUsage(dailyPrompts) {
    const dailyCredits = (dailyPrompts.simple * DEFAULT_CREDIT_COSTS['simple-prompt'] +
 ...
 */
export function suggestPlan(estimatedMonthlyCredits) {
