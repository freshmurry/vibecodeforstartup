...
import { setFileGenerating, appendFileChunk, setFileCompleted, setAllFilesCompleted, updatePhaseFileStatus, } from './file-state-helpers';
import { createAIMessage, handleRateLimitError, handleStreamingMessage, appendToolEvent, } from './message-helpers';
import { completeStages } from './project-stage-helpers';
import { sendWebSocketMessage } from './websocket-helpers';
import { toast } from 'sonner';
export function createWebSocketMessageHandler(deps) {
    return (websocket, message) => {
        const { setFiles, setPhaseTimeline, setProjectStages, setMessages, setBlueprint, setQuery, setPreviewUrl, setTotalFiles, setIsRedeployReady, setIsPreviewDeploying, setIsThinking, setIsInitialStateRestored, setShouldRefreshPreview, setIsDeploying, setCloudflareDeploymentUrl, setDeploymentError, setIsGenerationPaused, setIsGenerating, setIsPhaseProgressActive, isInitialStateRestored, blueprint, query, bootstrapFiles, files, phaseTimeline, previewUrl, projectStages, isGenerating, urlChatId, updateStage, sendMessage, loadBootstrapFiles, onDebugMessage, onTerminalMessage, } = deps;
        ...
        if (message.type !== 'file_chunk_generated' && message.type !== 'cf_agent_state' && message.type.length <= 50) {
            ...
            onDebugMessage?.('websocket', `${message.type}`, JSON.stringify(message, null, 2), 'WebSocket', message.type, message);
        }
        switch (message.type) {
            case 'cf_agent_state': {
                const { state } = message;
                ...
                if (!isInitialStateRestored) {
                    ...
                    if (state.generatedFilesMap && files.length === 0) {
                        setFiles(Object.values(state.generatedFilesMap).map((file) => ({
                    ...
                    if (state.generatedPhases && state.generatedPhases.length > 0 && phaseTimeline.length === 0) {
                        ...
                        const timeline = state.generatedPhases.map((phase, index) => ({
                            ...
                            status: phase.completed ? 'completed' : 'generating',
                            files: phase.files.map((filesConcept) => {
                                ...
                                return {
                                    ...
                                    status: (file ? 'completed' : 'generating'),
            ...
            case 'file_generated': {
                ...
                setPhaseTimeline((prev) => updatePhaseFileStatus(prev, message.file.filePath, 'completed', message.file.fileContents));
                break;
            }
...
Message: ${message.errors.map((e) => e.message).join('\n').trim()}`;
                ...
                onDebugMessage?.('error', `Runtime Error (${message.count} errors)`, message.errors.map((e) => `${e.message}\nStack: ${e.stack || 'N/A'}`).join('\n\n'), 'Runtime Detection');
                break;
            }
            case 'code_reviewing': {
                const totalIssues = (message.staticAnalysis?.lint?.issues?.length || 0) +
                ...
                if (totalIssues > 0) {
                    ...
                    onDebugMessage?.('warning', `Generation Issues Found (${totalIssues} total)`, errorDetails, 'Code Generation');
                }
                break;
            }
            ...
            case 'phase_implemented': {
                ...
                if (message.phase) {
                    setPhaseTimeline(prev => {
                        ...
                        if (updated.length > 0) {
                            ...
                            lastPhase.files = lastPhase.files.map(f => ({ ...f, status: 'completed' }));
                ...
                setTimeout(() => {
                    ...
                    onDebugMessage?.('info', 'Preview Auto-Refresh Triggered', `Preview refreshed 1 second after deployment completion`, 'Preview Auto-Refresh');
                }, 1000);
                break;
            }
            ...
            case 'cloudflare_deployment_completed': {
                ...
                onDebugMessage?.('info', 'Deployment Completed - Redeploy Reset', `Deployment URL: ${message.deploymentUrl}\nPhase count at deployment: ${phaseTimeline.length}\nRedeploy button disabled until next phase`, 'Redeployment Management');
                break;
            }
            case 'cloudflare_deployment_error': {
                ...
                onDebugMessage?.('error', 'Deployment Failed - State Reset', `Error: ${message.error}\nDeployment button reset for retry`, 'Deployment Error Recovery');
                break;
            }
            ...
            case 'conversation_response': {
                ...
                setMessages(prev => {
                    ...
                    if (idx !== -1)
                        return prev.map((m, i) => i === idx ? { ...m, message: message.message } : m);
            ...
            case 'rate_limit_error': {
                const rateLimitMessage = handleRateLimitError(message.error, onDebugMessage);
