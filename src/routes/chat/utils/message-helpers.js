 ...
 */
export function isConversationalMessage(messageId) {
 ...
 */
export function createAIMessage(id, message, isThinking) {
    return {
        type: 'ai',
        id,
        message,
 ...
 */
export function handleRateLimitError(rateLimitError, onDebugMessage) {
    ...
    onDebugMessage?.('error', `Rate Limit: ${rateLimitError.limitType.replace('_', ' ')} limit exceeded`, `Limit: ${rateLimitError.limit} per ${Math.floor((rateLimitError.period || 0) / 3600)}h\nRetry after: ${(rateLimitError.period || 0) / 3600}h\n\nSuggestions:\n${rateLimitError.suggestions?.join('\n') || 'None'}`, 'Rate Limiting', rateLimitError.limitType, rateLimitError);
    return createAIMessage(`rate_limit_${Date.now()}`, `⏱️ ${displayMessage}`);
}
/**
 * Add or update a message in the messages array
 */
export function addOrUpdateMessage(messages, newMessage, messageType = 'ai') {
    ...
    if (newMessage.id === 'main') {
        ...
        if (mainMessageIndex !== -1) {
            return messages.map((msg, index) => index === mainMessageIndex
 ...
 */
export function handleStreamingMessage(messages, messageId, chunk, isNewMessage) {
    const existingMessageIndex = messages.findIndex(m => m.id === messageId && m.type === 'ai');
    if (existingMessageIndex !== -1 && !isNewMessage) {
        // Append chunk to existing message
        return messages.map((msg, index) => index === existingMessageIndex
 ...
 */
export function appendToolEvent(messages, messageId, tool) {
        ...
        if (tool.status === 'success') {
            ...
            for (let j = current.length - 1; j >= 0; j--) {
                if (current[j].name === tool.name) {
                    return {
                        ...m,
                        toolEvents: current.map((ev, k) => k === j ? { ...ev, status: 'success', timestamp } : ev),
                    };
                }
            }
