...
import { RateLimitExceededError } from '@/api-types';
import { createRepairingJSONParser, ndjsonStream, } from '@/utils/ndjson-parser/ndjson-parser';
import { getFileType } from '@/utils/string';
import { logger } from '@/utils/logger';
import { apiClient } from '@/lib/api-client';
import { appEvents } from '@/lib/app-events';
import { createWebSocketMessageHandler } from '../utils/handle-websocket-message';
import { isConversationalMessage, addOrUpdateMessage, createUserMessage, handleRateLimitError } from '../utils/message-helpers';
import { sendWebSocketMessage } from '../utils/websocket-helpers';
import { initialStages as defaultStages, updateStage as updateStageHelper } from '../utils/project-stage-helpers';
export function useChat({ chatId: urlChatId, query: userQuery, agentMode = 'deterministic', onDebugMessage, onTerminalMessage, }) {
    ...
    const [projectStages, setProjectStages] = useState(defaultStages);
    ...
    const [cloudflareDeploymentUrl, setCloudflareDeploymentUrl] = useState('');
    ...
    const loadBootstrapFiles = (files) => {
    ...
    const connectWithRetry = useCallback((wsUrl, { disableGenerate = false, isRetry = false } = {}) => {
        ...
        try {
            ...
            const connectionTimeout = setTimeout(() => {
                ...
                if (myAttemptId !== connectAttemptIdRef.current)
                    return;
            ...
            ws.addEventListener('close', (event) => {
                clearTimeout(connectionTimeout);
                logger.info(`🔌 WebSocket connection closed with code ${event.code}: ${event.reason || 'No reason provided'}`, event);
    ...
    const handleConnectionFailure = useCallback((wsUrl, disableGenerate, reason) => {
        ...
        if (retryCount.current >= maxRetries) {
            ...
            onDebugMessage?.('error', 'WebSocket Connection Failed Permanently', `Failed after ${maxRetries + 1} attempts. Reason: ${reason}`, 'WebSocket Resilience');
            return;
        }
        retryCount.current++;
        ...
        onDebugMessage?.('warning', 'WebSocket Connection Retry', `Retry ${retryCount.current}/${maxRetries} in ${actualDelay / 1000}s. Reason: ${reason}`, 'WebSocket Resilience');
    }, [maxRetries, retryCount, retryTimeouts, onDebugMessage, sendMessage]);
    // No legacy wrapper; call connectWithRetry directly
    useEffect(() => {
        async function init() {
            if (!urlChatId || connectionStatus.current !== 'idle')
                return;
            try {
                if (urlChatId === 'new') {
                    if (!userQuery) {
    ...
    const handleDeployToCloudflare = useCallback(async (instanceId) => {
        try {
            ...
            if (sendWebSocketMessage(websocket, 'deploy', { instanceId })) {
                ...
                setTimeout(() => {
                    if (isDeploying) {
                        ...
                        onDebugMessage?.('warning', 'Deployment Timeout', `Deployment for ${instanceId} timed out after 60 seconds`, 'Deployment Timeout Management');
                    }
