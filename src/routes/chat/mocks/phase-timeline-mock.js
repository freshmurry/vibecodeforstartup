...
export function generateMockPhaseTimeline() {
    ...
    return [
        ...
        },
        {
            ...
            files: mockFiles.dashboardUI.map((f, index) => ({
                ...f,
                status: index < 2 ? 'completed' : 'generating'
            })),
            status: 'generating',
...
export function generateSequentialMockPhases() {
    ...
    return [
        ...
        // Phase 3: Currently implementing
        {
            ...
            files: mockFiles.gameLogic.map((f, index) => ({
                ...f,
                status: index < 3 ? 'completed' : 'generating'
            })),
            status: 'generating',
...
export function generateEmptyMockTimeline() {
    return [];
}
// Mock data with very long file names for testing truncation
export function generateLongFileNamesMockTimeline() {
...
export const mockScenarios = {
    ...
    'All Completed': () => generateMockPhaseTimeline().map(phase => ({
        ...phase,
        status: 'completed',
        files: phase.files.map(f => ({ ...f, status: 'completed' }))
    }))
};
