"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.FastCodeFixerOperation = void 0;
var common_1 = require("../inferutils/common");
var infer_1 = require("../inferutils/infer");
var prompts_1 = require("../prompts");
var common_2 = require("../operations/common");
var scof_1 = require("../streaming-formats/scof");
var SYSTEM_PROMPT = "You are a Senior Software Engineer at Cloudflare's Incident Response Team specializing in rapid bug fixes. Your task is to analyze identified code issues and generate complete fixed files using the SCOF format.";
var USER_PROMPT = "\n================================\nHere is the codebase of the project:\n<codebase>\n{{codebase}}\n</codebase>\n\nThis was the original project request from our client:\n<client_request>\n{{query}}\n</client_request>\n\nIdentified issues:\n<issues>\n{{issues}}\n</issues>\n================================\n\n## EXAMPLES OF COMMON FIXES:\n\n**Example 1 - Runtime Error Fix:**\nIssue: \"Cannot read property 'length' of undefined in GameBoard.tsx\"\nProblem: Missing null check for gameState\nSolution: Add conditional rendering and null checks\n\n**Example 2 - State Loop Fix:**\nIssue: \"Maximum update depth exceeded in ScoreDisplay.tsx\"\nProblem: useEffect without dependencies causing infinite updates\nSolution: Add proper dependency array and conditional logic\n\n**Example 3 - Import Error Fix:**\nIssue: \"Module not found: Can't resolve './utils/helpers'\"\nProblem: Incorrect import path\nSolution: Fix import path to match actual file structure\n\n## TASK:\nAnalyze each reported issue and generate complete file contents with fixes applied. Use SCOF format for output.\n\n## FIX GUIDELINES:\n- Address ONLY the specific issues reported\n- Preserve all existing functionality and exports\n- Use existing dependencies only\n- No TODO comments or placeholders\n- Focus on runtime errors, infinite loops, and import issues\n- Maintain original file structure and interfaces\n";
var userPromptFormatter = function (query, issues, allFiles, _allPhases) {
    var prompt = prompts_1.PROMPT_UTILS.replaceTemplateVariables(USER_PROMPT, {
        query: query,
        issues: issues.length > 0 ? JSON.stringify(issues, null, 2) : 'No specific issues reported - perform general code review',
        codebase: prompts_1.PROMPT_UTILS.serializeFiles(allFiles)
    });
    return prompts_1.PROMPT_UTILS.verifyPrompt(prompt);
};
var FastCodeFixerOperation = /** @class */ (function (_super) {
    __extends(FastCodeFixerOperation, _super);
    function FastCodeFixerOperation() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    FastCodeFixerOperation.prototype.execute = function (inputs, options) {
        return __awaiter(this, void 0, void 0, function () {
            var query, issues, allFiles, allPhases, env, logger, userPrompt, systemPrompt, codeGenerationFormat, messages, result, files;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        query = inputs.query, issues = inputs.issues, allFiles = inputs.allFiles, allPhases = inputs.allPhases;
                        env = options.env, logger = options.logger;
                        logger.info("Fixing issues for ".concat(allFiles.length, " files"));
                        userPrompt = userPromptFormatter(query, issues, allFiles, allPhases);
                        systemPrompt = SYSTEM_PROMPT;
                        codeGenerationFormat = new scof_1.SCOFFormat();
                        messages = [
                            (0, common_1.createSystemMessage)(systemPrompt),
                            (0, common_1.createUserMessage)(userPrompt + codeGenerationFormat.formatInstructions())
                        ];
                        return [4 /*yield*/, (0, infer_1.executeInference)({
                                env: env,
                                messages: messages,
                                agentActionName: "fastCodeFixer",
                                context: options.inferenceContext,
                            })];
                    case 1:
                        result = _a.sent();
                        files = codeGenerationFormat.deserialize(result.string);
                        return [2 /*return*/, files];
                }
            });
        });
    };
    return FastCodeFixerOperation;
}(common_2.AgentOperation));
exports.FastCodeFixerOperation = FastCodeFixerOperation;
