...
function initializeConfig() {
    const { CLOUDFLARE_ACCOUNT_ID, CLOUDFLARE_AI_GATEWAY, CLOUDFLARE_API_TOKEN, CLOUDFLARE_AI_GATEWAY_TOKEN, CLOUDFLARE_AI_GATEWAY_URL } = process.env;
...
const QUERIES = {
    ...
    providerRequests: (start, end, granularity = 'minute') => ({
    ...
    modelTokens: (start, end, granularity = 'minute') => ({
    ...
    chatProviderRequests: (start, end, chatId, granularity = 'minute') => ({
    ...
    chatModelTokens: (start, end, chatId, granularity = 'minute') => ({
    ...
    specificId: (start, end, metadataValue) => ({
        operationName: null,
        variables: {
            accountTag: CONFIG.ACCOUNT_TAG,
            gateway: CONFIG.GATEWAY,
...
function processProviderRequestData(data) {
...
function processModelTokenData(data) {
    const providerMap = new Map();
...
function displayModelBreakdown(providers, topN) {
...
function displayTopModels(models, topN) {
...
function displayResults(results, options) {
    ...
    results.forEach(result => {
        ...
        if (result.name.includes('Provider Request Analytics')) {
            ...
            if (totalRequests) {
                const { cost, cachedRequests, erroredRequests, uncachedTokensIn, uncachedTokensOut, cachedTokensIn, cachedTokensOut } = totalRequests.sum;
...
function getTimeRange(days = 1, granularity = 'hour') {
    const end = new Date();
    ...
    try {
        ...
        if (args.userId) {
            ...
            if (args.showProviders || args.showModels) {
                queries.push({ name: 'User Provider Request Analytics', query: QUERIES.providerRequests(timeRange.start, timeRange.end, granularity) }, { name: 'User Model Token Analytics', query: QUERIES.modelTokens(timeRange.start, timeRange.end, granularity) });
            }
        }
        else if (args.chatId) {
            queries.push({
            ...
            if (args.showProviders || args.showModels) {
                queries.push({ name: `Chat Provider Request Analytics (${args.chatId})`, query: QUERIES.chatProviderRequests(timeRange.start, timeRange.end, args.chatId, granularity) }, { name: `Chat Model Token Analytics (${args.chatId})`, query: QUERIES.chatModelTokens(timeRange.start, timeRange.end, args.chatId, granularity) });
            }
        }
        else {
            // Default analytics - always include legacy queries for compatibility
            queries.push({ name: 'Total Gateway Analytics', query: QUERIES.totalGateway(timeRange.start, timeRange.end) }, { name: 'User-Filtered Analytics', query: QUERIES.userFiltered(timeRange.start, timeRange.end) }, { name: 'Chat-Filtered Analytics', query: QUERIES.chatFiltered(timeRange.start, timeRange.end) });
            ...
            if (args.showProviders || args.showModels || args.topModels || (!args.userId && !args.chatId)) {
                queries.push({ name: 'Provider Request Analytics', query: QUERIES.providerRequests(timeRange.start, timeRange.end, granularity) }, { name: 'Model Token Analytics', query: QUERIES.modelTokens(timeRange.start, timeRange.end, granularity) });
            }
        }
        ...
        for (const { name, query } of queries) {
            ...
            try {
                ...
                let dataSummary = '';
                if (name.includes('Provider Request Analytics') && data?.data?.viewer?.accounts?.[0]?.data) {
                    const providerData = data.data.viewer.accounts[0].data;
                ...
                else if (name.includes('Model Token Analytics') && data?.data?.viewer?.accounts?.[0]?.data) {
                    const modelData = data.data.viewer.accounts[0].data;
