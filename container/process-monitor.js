...
import { DEFAULT_MONITORING_OPTIONS, getDataDirectory } from './types.js';
/**
...
class SimpleLogManager {
    ...
    appendCount = 0;
    static CHECK_INTERVAL = 100; // Check file size every 100 appends
    constructor(instanceId, maxLines = 1000, maxFileSize = 1024 * 1024) {
     ...
     */
    async appendLog(content, stream) {
        try {
...
class LogClassifier {
    ...
    static shouldStoreAsLog(content) {
...
class ErrorDetector {
    patterns;
    fastPatterns = new Map(); // Keyword -> patterns mapping
    severityPatterns = new Map(); // Severity-based grouping
    fallbackPatterns = []; // Catch-all patterns
    constructor() {
     ...
     */
    extractPatternKeywords(pattern) {
        const keywords = [];
    ...
    extractErrorInfo(content, pattern, match, context) {
     ...
     */
    handleClientError(jsonStr, content, context) {
        try {
     ...
     */
    cleanupClientErrorJson(jsonStr) {
     ...
     */
    extractFallbackClientErrorMessage(jsonStr) {
    ...
    createFallbackError(content, context) {
    ...
    inferCategory(content) {
        ...
        if (lower.includes('syntaxerror:') || lower.includes('unexpected token') || lower.includes('unexpected end') ||
            lower.includes('duplicate identifier') || lower.includes('assignment to constant variable'))
            return 'syntax';
        // React component export issues - very specific
        if (lower.includes('element type is invalid') ||
            (lower.includes('export') && lower.includes('component')))
            return 'dependency';
        ...
        if (lower.includes('cannot resolve module') || lower.includes('module not found') ||
            ...
            lower.includes('error: cannot resolve'))
            return 'dependency';
        // Build system errors
        if (lower.includes('[vite:build]') || lower.includes('rollup failed') || lower.includes('build failed') ||
            ...
            lower.includes('compilation failed') || lower.includes('could not resolve'))
            return 'compilation';
        // CSS parsing errors
        if (lower.includes('[vite:css]') || lower.includes('css') && (lower.includes('unexpected') || lower.includes('parse')))
            return 'syntax';
        // Then broader but still specific patterns
        if (lower.includes('module') && (lower.includes('resolve') || lower.includes('import')))
            return 'dependency';
        if (lower.includes('syntax') && !lower.includes('error:'))
            return 'syntax'; // Avoid "SyntaxError:" which is runtime
        if (lower.includes('compile') || lower.includes('build') || lower.includes('transform'))
            return 'compilation';
        if (lower.includes('memory') || lower.includes('heap'))
            return 'memory';
        if (lower.includes('network') || lower.includes('fetch') || lower.includes('connection'))
            return 'network';
        if (lower.includes('file') || lower.includes('path') || lower.includes('directory'))
            return 'filesystem';
        if (lower.includes('port') || lower.includes('env') || lower.includes('config'))
            return 'environment';
        return 'runtime';
    }
...
class LogBuffer {
    ...
    getRecentLines(count = this.size) {
        const result = [];
...
export class ProcessMonitor extends EventEmitter {
    ...
    constructor(processInfo, storage, options = {}) {
        super();
    ...
    errorBuffer = '';
    bufferTimeout;
    static MAX_ERROR_BUFFER_SIZE = 64 * 1024; // 64KB limit to prevent memory leaks
    // Stream processing throttling for high-throughput scenarios
    processingQueue = [];
    isProcessing = false;
    static MAX_QUEUE_SIZE = 100; // Prevent memory buildup during bursts
    /**
     * Process stream data with enhanced multi-line error handling and backpressure control
     */
    processStreamData(data, stream) {
     ...
     */
    bufferMultiLineErrors(content, stream) {
     ...
     */
    detectErrorsInOutputChunk(output, source) {
        if (!output.trim())
            return;
    ...
    handleDetectedError(error) {
        ...
        if (!isDuplicate) {
            ...
            const storeResult = this.storage.storeError(this.processInfo.instanceId, this.processInfo.id, truncatedError);
     ...
     */
    smartTruncateRawOutput(rawOutput) {
        if (!rawOutput)
            return rawOutput;
     ...
     */
    checkForDuplicateError(newError) {
     ...
     */
    areErrorsRelated(existing, newError) {
