...
import { DEFAULT_MONITORING_OPTIONS, DEFAULT_STORAGE_OPTIONS, DEFAULT_LOG_STORE_OPTIONS, getDataDirectory, getErrorDbPath, getLogDbPath } from './types.js';
/**
...
class SafeCleanup {
     ...
     */
    static async closeStorage(storage, timeoutMs = 2000) {
        if (!storage)
            return;
        try {
...
class OutputFormatter {
     ...
     */
    static formatError(error, additionalData) {
        const errorResponse = {
            success: false,
            error,
            ...additionalData
     ...
     */
    static printErrorsTable(errors) {
        if (errors.length === 0) {
...
class ErrorCommands {
    ...
    static async list(options) {
        ...
        try {
            ...
            if (options.categories) {
                filteredErrors = filteredErrors.filter(error => options.categories.includes(error.category));
            }
            if (options.severities) {
                filteredErrors = filteredErrors.filter(error => options.severities.includes(error.severity));
            }
            if (options.since) {
                const sinceDate = new Date(options.since);
                filteredErrors = filteredErrors.filter(error => new Date(error.lastOccurrence) >= sinceDate);
            }
            if (options.until) {
                const untilDate = new Date(options.until);
                filteredErrors = filteredErrors.filter(error => new Date(error.lastOccurrence) <= untilDate);
            }
            // Apply pagination
            ...
            try {
                OutputFormatter.formatError(error instanceof Error ? error.message : String(error), { instanceId: options.instanceId });
            }
            catch (formatError) {
    ...
    static countByField(errors, field) {
        const counts = {};
        for (const error of errors) {
 ...
 */
async function handleProcessCommand(subcommand, args, remainingArgs) {
    switch (subcommand) {
        case 'start':
 ...
 */
async function handleErrorCommand(subcommand, args) {
    switch (subcommand) {
        case 'list':
            if (!args['instance-id']) {
            ...
            await ErrorCommands.list({
                instanceId: String(args['instance-id']),
                categories: args.categories ? String(args.categories).split(',') : undefined,
                severities: args.severities ? String(args.severities).split(',') : undefined,
 ...
 * Handle log commands
 */
async function handleLogCommand(subcommand, args) {
    switch (subcommand) {
        case 'list':
            if (!args['instance-id']) {
                OutputFormatter.formatError('--instance-id is required for list command');
            ...
            await LogCommands.list({
                instanceId: String(args['instance-id']),
                levels: args.levels ? String(args.levels).split(',') : undefined,
                streams: args.streams ? String(args.streams).split(',') : undefined,
                since: args.since ? String(args.since) : undefined,
                until: args.until ? String(args.until) : undefined,
                limit: args.limit ? parseInt(String(args.limit)) : undefined,
                offset: args.offset ? parseInt(String(args.offset)) : undefined,
