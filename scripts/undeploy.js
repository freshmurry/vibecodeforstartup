...
class CloudflareUndeploymentManager {
    ...
    parseWranglerConfig() {
        ...
        try {
            ...
            throw new UndeploymentError('Failed to parse wrangler.jsonc file', error instanceof Error ? error : new Error(String(error)));
        }
    }
    /**
     * Validate wrangler command for security
     */
    validateWranglerCommand(command) {
     ...
     */
    execWranglerCommand(command, description) {
        try {
     ...
     */
    async execWranglerCommandAsync(command, description) {
        try {
            console.log(`🔄 ${description}...`);
    ...
    async deleteKVNamespaces() {
        ...
        const deletePromises = this.config.kv_namespaces.map(kvNamespace => this.execWranglerCommandAsync(`kv namespace delete --namespace-id=${kvNamespace.id}`, `Deleting KV namespace: ${kvNamespace.binding} (ID: ${kvNamespace.id})`));
    ...
    async deleteR2Buckets() {
        ...
        const deletePromises = this.config.r2_buckets.map(bucket => this.execWranglerCommandAsync(`r2 bucket delete ${bucket.bucket_name}`, `Deleting R2 bucket: ${bucket.bucket_name}`));
        const results = await Promise.allSettled(deletePromises);
        const successCount = results.filter(result => result.status === 'fulfilled' && result.value).length;
    ...
    async deleteD1Database() {
        ...
        const deletePromises = this.config.d1_databases.map(database => this.execWranglerCommandAsync(`d1 delete ${database.database_name} --skip-confirmation`, `Deleting D1 database: ${database.database_name}`));
        const results = await Promise.allSettled(deletePromises);
        const successCount = results.filter(result => result.status === 'fulfilled' && result.value).length;
    ...
    async deleteDispatchNamespace() {
        ...
        const deletePromises = this.config.dispatch_namespaces.map(dispatchNs => this.execWranglerCommandAsync(`dispatch-namespace delete ${dispatchNs.namespace}`, `Deleting dispatch namespace: ${dispatchNs.namespace}`));
        const results = await Promise.allSettled(deletePromises);
        const successCount = results.filter(result => result.status === 'fulfilled' && result.value).length;
    ...
    async deleteContainers() {
        ...
        try {
            ...
            for (const container of containers) {
                ...
                const isOurContainer = containerPatterns.some(pattern => container.name.toLowerCase().includes(pattern.toLowerCase()));
                if (isOurContainer) {
            ...
            const deletePromises = containersToDelete.map(container => this.execWranglerCommandAsync(`containers delete ${container.id}`, `Deleting container: ${container.name} (${container.id})`));
            const results = await Promise.allSettled(deletePromises);
            const successCount = results.filter(result => result.status === 'fulfilled' && result.value).length;
    ...
    async deleteContainerImages() {
        ...
        try {
            ...
            const deletePromises = imagesToDelete.map(imageRef => this.execWranglerCommandAsync(`containers images delete ${imageRef}`, `Deleting container image: ${imageRef}`));
            const results = await Promise.allSettled(deletePromises);
            const successCount = results.filter(result => result.status === 'fulfilled' && result.value).length;
