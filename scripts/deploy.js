...
class CloudflareDeploymentManager {
    ...
    env;
    cloudflare;
    aiGatewayCloudflare; // Separate SDK instance for AI Gateway operations
    conflictingVarsForCleanup = null; // For signal cleanup
    constructor() {
    ...
    validateEnvironment() {
        ...
        if (missingVars.length > 0) {
            throw new DeploymentError(`Missing required build variables: ${missingVars.join(', ')}\n` +
    ...
    parseWranglerConfig() {
        ...
        if (!existsSync(wranglerPath)) {
            throw new DeploymentError('wrangler.jsonc not found', new Error('Please ensure wrangler.jsonc exists in the project root'));
        }
        try {
            ...
            throw new DeploymentError('Failed to parse wrangler.jsonc', error instanceof Error ? error : new Error(`Please check your wrangler.jsonc syntax: ${String(error)}`));
        }
    }
    /**
    ...
    async ensureDispatchNamespace() {
        ...
        try {
            ...
            try {
                await this.cloudflare.workersForPlatforms.dispatch.namespaces.get(namespaceName, { account_id: this.env.CLOUDFLARE_ACCOUNT_ID });
            ...
            throw new DeploymentError(`Failed to ensure dispatch namespace: ${namespaceName}`, error instanceof Error ? error : new Error(String(error)));
        }
    }
    /**
     * Creates or ensures AI Gateway exists (non-blocking)
     */
    async ensureAIGateway() {
        ...
        try {
            ...
            commandsToUpdate.forEach((command) => {
                if (packageJson.scripts[command]) {
                    ...
                    const newCommand = oldCommand.replace(/wrangler d1 migrations apply [^\s]+ /, `wrangler d1 migrations apply ${databaseName} `);
            ...
            console.warn(`⚠️  Could not update package.json database commands: ${error instanceof Error ? error.message : String(error)}`);
     ...
     */
    async detectZoneForDomain(customDomain, originalDomain) {
     ...
     */
    updateWranglerField(content, field, value) {
     ...
     */
    updateWranglerForWorkersDev(content) {
     ...
     */
    updateWranglerForCustomDomain(content, routes, preserveExistingFlags = false) {
        let updatedContent = content;
        // Update routes
     ...
     */
    async safeDetectZoneForDomain(customDomain, originalCustomDomain) {
        try {
            ...
            console.warn(`⚠️  Zone detection failed for custom domain ${customDomain}: ${error instanceof Error ? error.message : String(error)}`);
            console.log('   → Continuing without zone-specific routes');
            return { zoneName: null, zoneId: null, success: false };
        }
    }
    async updateCustomDomainRoutes() {
        ...
        try {
            ...
            console.warn(`⚠️  Could not update custom domain routes: ${error instanceof Error ? error.message : String(error)}`);
            // Non-blocking - continue deployment
        }
    }
    /**
    ...
    updateContainerConfiguration() {
        ...
        const maxInstances = process.env.MAX_SANDBOX_INSTANCES ||
            this.config.vars?.MAX_SANDBOX_INSTANCES || "10";
        if (!maxInstances) {
            console.log('ℹ️  MAX_SANDBOX_INSTANCES not set in environment variables or wrangler.jsonc vars, skipping container configuration update');
            return;
        }
        ...
        try {
            ...
            const sandboxContainerIndex = config.containers.findIndex((container) => container.class_name === 'UserAppSandboxService');
            ...
            const edits = modify(content, ['containers', sandboxContainerIndex, 'max_instances'], maxInstancesNum, CloudflareDeploymentManager.JSONC_FORMAT_OPTIONS);
            ...
            throw new DeploymentError('Failed to update container configuration', error instanceof Error ? error : new Error(String(error)));
        }
    }
    /**
    ...
    updateContainerInstanceTypes() {
        // Environment variable takes priority over wrangler.jsonc vars
        const sandboxInstanceType = process.env.SANDBOX_INSTANCE_TYPE ||
        ...
        try {
            ...
            const userAppContainerIndex = config.containers.findIndex((container) => container.class_name === 'UserAppSandboxService');
            if (userAppContainerIndex === -1) {
                console.warn('⚠️  UserAppSandboxService container not found in wrangler.jsonc');
                return;
            }
            // Determine the instance type configuration
            let userAppInstanceType;
            ...
            let updatedContent = content;
            const userAppInstanceTypeEdits = modify(updatedContent, ['containers', userAppContainerIndex, 'instance_type'], userAppInstanceType, CloudflareDeploymentManager.JSONC_FORMAT_OPTIONS);
            ...
            this.logWarning(`Could not update container instance types: ${error instanceof Error ? error.message : String(error)}`, ['Continuing with current configuration...']);
            // Non-blocking - continue deployment
        }
    }
    /**
     ...
     */
    updateDispatchNamespace(dispatchNamespacesAvailable) {
        ...
        if (!dispatchNamespacesAvailable) {
            ...
            try {
                ...
                const varsEdits = modify(content, ['vars', 'DISPATCH_NAMESPACE'], '', CloudflareDeploymentManager.JSONC_FORMAT_OPTIONS);
                ...
                this.logWarning(`Could not clear DISPATCH_NAMESPACE var: ${error instanceof Error ? error.message : String(error)}`, ['Continuing with deployment...']);
            }
            return;
        }
        // Environment variable takes priority over wrangler.jsonc vars
        ...
        if (!namespacePattern.test(dispatchNamespace)) {
            console.warn(`⚠️  Invalid DISPATCH_NAMESPACE format: ${dispatchNamespace}, must contain only letters, numbers, hyphens, and underscores`);
            return;
        }
        ...
        try {
            ...
            const namespaceEdits = modify(updatedContent, ['dispatch_namespaces', 0, 'namespace'], dispatchNamespace, CloudflareDeploymentManager.JSONC_FORMAT_OPTIONS);
            updatedContent = applyEdits(updatedContent, namespaceEdits);
            // Update vars.DISPATCH_NAMESPACE for consistency
            const varsEdits = modify(updatedContent, ['vars', 'DISPATCH_NAMESPACE'], dispatchNamespace, CloudflareDeploymentManager.JSONC_FORMAT_OPTIONS);
            ...
            this.logWarning(`Could not update dispatch namespace configuration: ${error instanceof Error ? error.message : String(error)}`, ['Continuing with current configuration...']);
            // Non-blocking - continue deployment
        }
    }
    /**
    ...
    async wranglerDeploy() {
        ...
        try {
            ...
            throw new DeploymentError('Failed to deploy with Wrangler', error instanceof Error ? error : new Error(String(error)));
        }
    }
    /**
     ...
     */
    async restoreOriginalVars(originalConflictingVars) {
    ...
    createProdVarsFile() {
        ...
        try {
            ...
            throw new DeploymentError('Failed to create .prod.vars file', error instanceof Error ? error : new Error(String(error)));
        }
    }
    /**
     ...
     */
    async checkDispatchNamespaceAvailability() {
